...
import { getUserRestaurantsToTryList, getUserRestaurantsTriedList } from "@/firebase";

const GOOGLE_PLACES_API_KEY = process.env.EXPO_PUBLIC_GOOGLE_MAPS_API_KEY || "";

const yourLists = () => {
  const userId = JSON.parse(useLocalSearchParams<any>().userId as string);
  const { userTriedRestaurants, userToTryRestaurants, userDbInfo } = useAppStore();
  ...
  useEffect(() => {
    if (userId === userDbInfo?.id) {
      ...
      (async () => {
        setLoading(true);
        let location = await Location.getCurrentPositionAsync({});
        const triedRestaurantList = await getUserRestaurantsTriedList(userId);
        const toTryRestaurantList = await getUserRestaurantsToTryList(userId);

        const toTryData = await Promise.all(
          toTryRestaurantList.map(async (restaurantId) => {
            const data = await fetchRestaurant(restaurantId);
            const newRestaurant = {
              placeId: data.id,
              coordinate: {
        ...
        const triedData = await Promise.all(
          triedRestaurantList.map(async (restaurant) => {
            const data = await fetchRestaurant(restaurant.id);
            const newRestaurant = {
              placeId: data.id,
              coordinate: {
  ...
  }, [viewToTry, filteredTriedRestaurants, filteredToTryRestaurants, loading]);
  ...
  const performFilter = () => {
    ...
    if (restaurantListFilter.sortBy === "Distance") {
      ...
      data.sort((a, b) => {
        const restaurantA = originalTriedRestaurants.find((r) => r.id === a.placeId);
        const restaurantB = originalTriedRestaurants.find((r) => r.id === b.placeId);
    ...
    if (!viewToTry) {
      data = data.filter((restaurant) => {
        const foundRestaurant = originalTriedRestaurants.find((r) => r.id === restaurant.placeId);
        if (
